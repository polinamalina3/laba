...
public class NestandartKv implements ActionListener {
	JFrame frame = new JFrame("Расчет планировки нестандартной комнаты"); // главное окно
	...
	public static JTextField[] arrTf = new JTextField[15];// массив текстовых полей
...
//добавлям метки через метод addLabel
		...
		addLabel(panelLeft, "Длина выступающей ч.:", Color.BLACK);
		addLabel(panelLeft, "Ширина выступающей ч.:", Color.BLACK);
...
//метод расчета
	public double planirovka(double kolOb, double kolPl) throws Exception {
		...
		if ((dlinaDver != 0) & (shirinaDver != 0) & (vysotaKomn != 0)) {
			q1 = 2 * vysotaKomn * (dlinaKomn + shirinaKomn + dlinaVystup + shirinaVystup); // Стены квартиры
		}
		...
		if ((dlinaOb != 0) & (shirinaOb != 0)) {
			q5 = ((dlinaKomn * shirinaKomn) - (dlinaVystup * shirinaVystup)) / (dlinaPl * shirinaPl);// Количество плитки
			kolPl = Math.round(q5);
		}
